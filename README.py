...
# # Como configurar/instalar/usar o `shell-genie` no `Linux Ubuntu`
# 
# ## Resumo
# 
# Neste documento estão contidos os principais comandos e configurações para configurar/instalar/usar o `shell-genie` no `Linux Ubuntu`.
# 
# ## _Abstract_
# 
# _This document contains the main commands and settings for configuring/installing/using the `shell-genie` on `Linux Ubuntu`._
...
# ## 1. Como configurar/instalar/usar o `shell-genie` no `Linux Ubuntu` [1][3]
# 
# Para configurar/instalar/usar o `shell-genie` no `Linux Ubuntu`, você pode seguir estes passos:
# 
...
# Para instalar o programa `shell-genie`, que é um aplicativo de webcam, no Linux Ubuntu utilizando o Terminal Emulator, você pode seguir os passos abaixo:
# 
...
# 
# Para configurar/instalar/usar o `shell-genie` no `Linux Ubuntu` sem precisar digitar linha por linha, você pode seguir estas etapas:
# 
...
#     sudo apt autoclean -y
#     sudo apt install shell-genie -y
#     shell-genie
#     ```
...
# ## Referências
# 
# [1] OPENAI. ***Install shell-genie on Ubuntu.*** Disponível em: <https://chat.openai.com/share/shell_genie_ubuntu_install> (texto adaptado). Acessado em: 23/04/2023 17:11.
# 
